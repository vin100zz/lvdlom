  ...
  return {
    ...
    controller: function ($scope) {
      ...
      var render = function () {
        ...
        $scope.mainChart = new Chart(document.querySelector("#charts .main").getContext("2d")).Doughnut($scope.mainData, options);
        
        if ($scope.innerChart) {
          $scope.innerChart.destroy();
        }
        $scope.innerData = prepareData($scope.cfg.inner);
        $scope.innerChart = new Chart(document.querySelector("#charts .inner").getContext("2d")).Pie($scope.innerData, optionsInner);
      };
